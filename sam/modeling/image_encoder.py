...
from typing import Type, Tuple, Optional, Union, List, Dict
...
def get_rel_pos(q_size: int, k_size: int, rel_pos: Tensor) -> Tensor:
    ...
    if rel_pos.shape[0] != max_rel_dist:
        # Interpolate rel pos.
        rel_pos_resized = ops.upsampling2d(scale_factor=max_rel_dist, mode="bilinear")(
            ops.permute021()(ops.reshape()(rel_pos, [1, rel_pos.shape()[0], -1]))
        )
        rel_pos_resized = ops.permute()(ops.reshape()(rel_pos_resized, [-1, max_rel_dist]), [1, 0])
    else:
...
class Attention(nn.Module):
    ...
    def __init__(
        ...
        if self.use_rel_pos:
            ...
            self.rel_pos_h = zeros([2 * input_size[0] - 1, head_dim])
            self.rel_pos_w = zeros([2 * input_size[1] - 1, head_dim])

    def forward(self, x: Tensor) -> Tensor:
        B, H, W, _ = ops.size()(x)
        # qkv with shape (3, B, nHead, H * W, C)
        qkv = self.qkv(x)
        _h = [H.lower_bound(), H.upper_bound()]
        _w = [W.lower_bound(), W.upper_bound()]
        hw = [_h[0] * _w[0], _h[1] * _w[1]]
        hw = IntVar(hw)
        qkv = ops.reshape()(qkv, [B, hw, 3, self.num_heads, -1])
        qkv = ops.permute()(qkv, [2, 0, 3, 1, 4])
        # q, k, v with shape (B * nHead, H * W, C)
        qkv = ops.reshape()(qkv, [3, B * self.num_heads, hw, -1])
        q, k, v = ops.split()(qkv, 3, dim=0)

        attn = (q * self.scale) @ ops.transpose()(k, -2, -1)

        if self.use_rel_pos:
            attn = add_decomposed_rel_pos(attn, q, self.rel_pos_h, self.rel_pos_w, (H, W), (H, W))
        
        attn = ops.softmax()(attn, dim=-1)
        x = ops.reshape()(ops.permute()(ops.reshape()((attn @ v), [B, self.num_heads, H, W, -1]), [0, 2, 3, 1, 4]), [B, H, W, -1])
        x = self.proj(x)
...
class ImageEncoderViT(nn.Module):
    def __init__(
        ...
        if use_abs_pos:
            ...
            self.pos_embed = zeros([1, img_size // patch_size, img_size // patch_size, embed_dim])